...
export namespace RefreshTokenModel {
  ...
  export async function Insert({_id, userId}: {_id: string, userId: string}): Promise<RefreshToken> {
    ...
    await dbProvider.Insert('m_refresh_tokens', {_id, userId, timestamp: now});
    const result = await dbProvider.Find<RefreshToken>('m_refresh_tokens', {_id, userId, timestamp: now});
    if (result.length == 1)
      return result[0]
    else
      return undefined;
  ...
  export async function Find({_id}: {_id: string}): Promise<RefreshToken> {
    const result = await dbProvider.Find<RefreshToken>('m_refresh_tokens', {_id});
    if (result.length == 1)
      return result[0]
    else
      return undefined;
