...
export class AuthService {
  async Login({ username, password }: { username: string, password: string }): Promise<IResult> {
    try {
      ...
      if (!user.confirmed)
        return {success: false, error: {status: 401, message: 'account not confirmed'}};
  ...
  async Register({ username, password }: { username: string, password: string }): Promise<IResult> {
    try {        
      if (!username)
        return {success: false, error: {status: 404, message: '\'username\' is missing'}};
      ...
      let result = await UserModel.CreateUser({ username, password, salt: newSalt });      
      
      return {success: true, data: {user: GetUser_O(result[0]), message: 'User is created. Please contact the admin to activate your account'}};
    } catch (error) {
      return {success: false, error: {status: 500, message: 'Unexpected Server Error', internalMessage: error}};
    }    
  }
