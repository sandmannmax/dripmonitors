...
export class ZalandoMonitor {
  public static GetProducts = async function ({ proxy }: { proxy: Proxy }): Promise<Array<Product>> {
    ...
    for (let i = 0; i < 3; i++) {
      const url = 'https://www.zalando.de/herrenschuhe-sneaker/?p=' + i;
      ...
      if (!articles) logger.info(`no articles ${  proxy.id  } `);// + userAgent);
      else {
        let hasSizes = false;
          ...
          for (let j = 0; j < articles.length; j++) {
            ...
            if (articles[j].family_articles.length > 1) {
              for (let k = 1; k < articles[j].family_articles.length; k++) {
                items.push(
                  ZalandoMonitor.GetProduct(articles[j].family_articles[k])
                );
              }
            }
          }
  ...
  private static GetProduct = function (article): Product {
    ...
    const price = article.price.promotional;
    product.price =
      `${price.substr(0, price.length - 3).replace(',', '.')  } EUR`;
    product.active = true;
    product.sizes = [];
    ...
    if (article.sizes) {
      ...
      for (let i = 0; i < product.sizes.length; i++) product.sizesSoldOut.push(false);
    } else {
