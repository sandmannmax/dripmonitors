...
} from '../proto/scraper/v1/scraper_pb';
...
export const Run = async function ({
  ...
  const monitorrun = new Monitorrun();

  try {
    const canStart = await MonitorpageModel.Start({ id });
    ...
    const isMonitorpageVisible = await MonitorpageModel.IsVisible({ id });

    for (let i = 0; i < products.length; i++) {
      let product = products[i];
      const oldProduct = await ProductModel.GetProduct({ id: product.id });
      ...
      if (oldProduct == null) {
        ...
        if (product) {
          ...
          if (product.active && !product.soldOut) {
            // If active and not sold out send all sizes
            sendMessage = true;
            for (let j = 0; j < product.sizes.length; j++) {
              if (!product.sizesSoldOut[j]) size += `${product.sizes[j]} - `;
            }
            if (size.length > 3) { size = size.substr(0, size.length - 3); }
          }
        }
      } else {
        ...
        if (product.active && !product.soldOut) {
          // If active and not sold out then
          if (!oldProduct.active || oldProduct.soldOut) {
            // If the old Product was inactive or soldout send all and update
            ...
            if (size.length > 3) { size = size.substr(0, size.length - 3); }
          } else if (oldProduct.active && !oldProduct.soldOut) {
            // If the old Product also was active and not sold out then
            let update = false;

            for (let j = 0; j < product.sizes.length; j++) {
              // Iterate over all sizes in new Product
              const index = oldProduct.sizes.findIndex((item) => item == product.sizes[j]); // Find index of same size

              if (index != -1) {
                // if same size was found on old Product
                if (product.sizesSoldOut[j] != oldProduct.sizesSoldOut[index]) {
                  // and the soldout state changed
                  update = true; // then update

                  if (!product.sizesSoldOut[j]) {
                    // if its not sold out then send size with message
                    sendMessage = true;
                    size += `${product.sizes[j]} - `;
                  }
                }
              } else {
                // if new size then update
                update = true;

                if (!product.sizesSoldOut[j]) {
                  // if this size is not sold out sent size with message
                  sendMessage = true;
                  size += `${product.sizes[j]} - `;
                }
              }
            ...
            for (let j = 0; j < oldProduct.sizes.length; j++) {
              // Iterate over all sizes in old Product
              const index = product.sizes.findIndex((item) => item == oldProduct.sizes[j]); // Find index of same size

              if (index == -1) {
                // if size is not in sizes of new product, add size and set to soldOut
          ...
          if (product.active != oldProduct.active) { update = true; }

          if (product.soldOut != oldProduct.soldOut) { update = true; }
      ...
      if (sendMessage && isMonitorpageVisible) {
        const monitors = await MonitorModel.GetMonitors({
          product,
          monitorpageId: id,
        });
        DiscordService.SendMessage({
          monitors, product, size, page: name, 
        });
      }
    }
...
function GetProducts({ techname, proxy }): Promise<Product[]> {
  return new Promise<Product[]>((resolve, reject) => {
    ...
    const request: GetProductsRequest = new GetProductsRequest();
    ...
    client.getProducts(request, (error, response) => {
      if (error) reject(error);
        ...
        const productsGRPC = response.getProductsList();
